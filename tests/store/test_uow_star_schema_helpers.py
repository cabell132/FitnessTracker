"""UnitOfWork helpers for star-schema SQL and tracker linking (GH issue 8).
...
def test_should_return_only_tc_items_missing_hevy_links_when_mixed_items_exist(
    ...
    with store.unit_of_work() as uow:
        uow.add(TrueCoachWorkout(id=700, title="W", state="scheduled", rest_day=False))
        uow.add(HevyAppWorkout(id="700", title="H", description="", start_time=dt, end_time=dt))
    ...
    with store.unit_of_work() as uow:
        rows = uow.get_unlinked_tc_workout_items(700)
...
def test_should_return_only_hevy_items_not_yet_on_workout_items(store: Store) -> None:
    ...
    with store.unit_of_work() as uow:
        uow.add(TrueCoachWorkout(id=701, title="W2", state="scheduled", rest_day=False))
        uow.add(HevyAppWorkout(id="701", title="H2", description="", start_time=dt, end_time=dt))
    ...
    with store.unit_of_work() as uow:
        rows = uow.get_unlinked_hevy_workout_items(701)
...
def test_should_set_hevy_app_id_when_link_workout_item_hevy_id_called(store: Store) -> None:
    ...
    with store.unit_of_work() as uow:
        uow.add(TrueCoachWorkout(id=702, title="W3", state="scheduled", rest_day=False))
...
def test_should_noop_link_workout_item_hevy_id_when_row_missing(store: Store) -> None:
    ...
    with store.unit_of_work() as uow:
        uow.link_workout_item_hevy_id(true_coach_id=999999, hevy_app_id=1)
...
def test_should_set_true_coach_id_on_metric_item_when_linked(store: Store) -> None:
    ...
    with store.unit_of_work() as uow:
        uow.add(TrueCoachAssessment(id=400, assessment_group_id=1, name="m", units="kg", order=0))
        uow.add(Metric(id=500, name="weight", true_coach_id=400))
        uow.flush()
        uow.add(MetricItem(id=600, metric_id=500, value=80.0, date=datetime(2025, 3, 1)))  # noqa: DTZ001

    with store.unit_of_work() as uow:
        uow.link_metric_item_to_true_coach(metric_item_id=600, true_coach_id=9001)
...
def test_should_noop_link_metric_item_when_row_missing(store: Store) -> None:
    ...
    with store.unit_of_work() as uow:
        uow.link_metric_item_to_true_coach(metric_item_id=999999, true_coach_id=1)
...
def _seed_workout_item_exercise_update_scenario(store: Store) -> None:
    ...
    with store.unit_of_work() as uow:
        uow.add(TrueCoachWorkout(id=703, title="W4", state="scheduled", rest_day=False))
        uow.add(HevyAppWorkout(id="703", title="H4", description="", start_time=dt, end_time=dt))
        uow.add(HevyAppExercise(id="hex99", name="Pull", type="w", equipment="bar", default=False))
...
def test_should_update_workout_item_exercise_ids_from_hevy_links(store: Store) -> None:
    ...
    with store.unit_of_work() as uow:
        uow.update_workout_exercise_ids_from_hevy(703)
