...
class HevyToFitnessTrackerSyncronizer:
    ...
    def update_workout(self, uow: Tx, workout: UpdatedWorkout) -> None:
        ...
        with WideEvent(
            ...
            uow.hevy.add_workout(workout.workout)
    ...
    def delete_workout(self, uow: Tx, event: DeletedWorkout) -> None:
        ...
        """
        uow.hevy.delete_workout(id=event.id)
    ...
    def link_workout_items(self, uow: Tx, true_coach_id: int) -> None:
        ...
        """
        uow.cross_domain.link_hevy_tracker_workout_items(true_coach_id)
        uow.session.flush()

        true_coach_items = uow.cross_domain.get_unlinked_tc_workout_items(true_coach_id)
        hevy_items = uow.cross_domain.get_unlinked_hevy_workout_items(true_coach_id)
        ...
        for link in link_list.links:
            if link.hevy_app_id is None or link.true_coach_id is None:
                continue
            uow.tracker.link_workout_item_hevy_id(link.true_coach_id, link.hevy_app_id)
        uow.session.flush()

    def update_exercise(self, uow: Tx, true_coach_id: int) -> None:
        ...
        """
        uow.cross_domain.update_workout_exercise_ids_from_hevy(true_coach_id)

    def update_sets(self, uow: Tx, true_coach_id: int) -> None:
        ...
        Args:
            ...
            true_coach_id (int): True Coach workout id scope.
        """
        uow.cross_domain.update_hevy_tracker_sets(true_coach_id)

    def insert_sets(self, uow: Tx, true_coach_id: int) -> None:
        ...
        Args:
            ...
            true_coach_id (int): True Coach workout id scope.
        """
        uow.cross_domain.insert_hevy_tracker_sets(true_coach_id)

    def update_exercises(self, uow: Tx, true_coach_id: int) -> None:
        ...
        Args:
            ...
            true_coach_id (int): True Coach workout id scope.
        """
        uow.cross_domain.update_hevy_tracker_exercises(true_coach_id)
    ...
    def link_exercises(
        ...
        """
        true_coach_workout_items = uow.true_coach.get_workout_items(workout_id=true_coach_id)
    ...
    def link_exercise(
        ...
        """
        instance = uow.tracker.get_exercise(
        ...
        instance = uow.tracker.get_exercise(true_coach_id=best_match.id)
        if instance:
            instance.hevy_app_id = hevy_exercise.exercise_template_id
            uow.session.merge(instance)
    ...
    def update_metrics(self, uow: Tx) -> None:
