-- INSERT INTO "MetricItem"(metric_id, value, date, apple_id)
-- SELECT 1 as metric_id, value, timestamp, id
-- FROM "AppleHealthDataRecord"
-- WHERE data_type_id = 119

INSERT INTO "MetricItem"(metric_id, value, date, workout_id)
SELECT
	2 as metric_id,
    ROUND(CAST(COALESCE(SUM(a.value), 0) AS NUMERIC), 0) AS value,
    w.end_date AS date,
    w.id AS workout_id
FROM
    "Workout" w
